...
## 7. Remove Temporary Design Preview Tooling

The browser-only design preview is temporary MVP support tooling and should be removable without touching the app runtime.

Current temporary tooling:

- `tools/design-preview/`
- root npm script: `preview:design`

Removal goals:

- deleting the design preview must not require changes in `App.tsx`
- deleting the design preview must not require changes in `src/`
- deleting the design preview must not affect navigation, store initialization, SQLite, notifications, or production builds

Removal steps after MVP 1:

1. Delete `tools/design-preview/`
2. Remove the `preview:design` script from the root `package.json`
3. Remove design-preview-specific README notes if they are no longer useful
4. Verify the Expo app still boots and the dev-only app preview catalog still works
...
- keep all raw design-file browser preview logic outside `src/`
- do not let production app code import from `tools/design-preview/`
- if new temporary design-preview helpers are added before MVP 1, keep them inside that isolated tooling folder
